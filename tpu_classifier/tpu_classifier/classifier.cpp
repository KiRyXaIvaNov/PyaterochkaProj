...
std::string classify_request(const std::string& input_json_str) {
    ...
    // Жесткие словари-маркеры для ИТ-сервисов ТПУ (из ТЗ и специфики вуза)
    std::unordered_map<std::string, std::vector<std::string>> tpu_catalog = {
        {"Wi-Fi", {"интернет", "вайфай", "wifi", "tpu-student", "tpu-guest", "сеть", "подключиться"}},
        {"VPN", {"впн", "vpn", "anyconnect", "шлюз", "удаленка"}},
        {"корпоративная почта", {"почта", "outlook", "аутлук", "письмо", "ящик", "mail", "tpu.ru"}},
        {"оборудование", {"компьютер", "монитор", "принтер", "мышка", "клавиатура", "ноутбук", "кабель"}},
        {"программное обеспечение", {"виндовс", "windows", "офис", "office", "лицензия", "программа", "софт"}}
    };
